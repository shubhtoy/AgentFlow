...
const { parseRoot, classifyResource, identifyPrimaryFile } = require(path.join(srcDir, 'parser'));
const { validate } = require(path.join(srcDir, 'validator'));
const { exportWorkflow } = require(path.join(srcDir, 'exporter'));
...
const { RESERVED_DIRS, CANONICAL_CATEGORIES, TAXONOMY_REGISTRY } = require(path.join(srcDir, 'taxonomy'));
const { calculateTokens, formatTokenSummary } = require(path.join(srcDir, 'token-calculator'));
const { dryRun, formatDryRunTrace } = require(path.join(srcDir, 'dry-runner'));
...
const { searchRegistry, getServer } = require(path.join(srcDir, 'mcp', 'registry-client'));
