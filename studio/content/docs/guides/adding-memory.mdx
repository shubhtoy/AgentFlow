...
Memory is the only resource category the agent can write to during execution. While instructions, capabilities, and runbooks are read-only context, memory files accumulate knowledge across runs — architecture decisions, user preferences, and project facts.
...
<Callout type="warn">
Router nodes should read memory sparingly and never write to it. Routers evaluate conditions and route — nothing more.
</Callout>
</Step>
