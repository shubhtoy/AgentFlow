...
```
build-feature: 4 errors, 2 warnings

  [error] broken_ref: Reference "instructions/codestyle" not found
    in build-feature/implement/SKILL.md

  [error] missing_condition: Conditional edge references "runbooks/approved"
    but no such runbook exists
    in build-feature/review-gate/SKILL.md
...
Four errors block export. Fix them in this order: broken references first (they often cause cascading issues), then structural problems.
</Step>
...
<Step>
### Fix missing_condition — create the runbook

The gate references `runbooks/approved` but the file does not exist. Either the file was deleted or never created.

Create `.agentflow/runbooks/approved.md`:

```yaml
---
type: condition
name: approved
check: The user explicitly approves the work
---

# Approved

**True when:** The user says the work is acceptable.
**False when:** The user requests changes or has not responded.
```

Alternatively, if the runbook exists under a different name (like `design-approved`), update the edge reference in the gate's SKILL.md to match:
...
<Tabs items={['References', 'Graph', 'Context', 'MCP']}>
  <Tab value="References">
    ...
    | `missing_condition` | Conditional edge references a missing runbook | Create the runbook file or fix the reference |
    ...
    | `unknown_category` | Reference uses a non-standard directory | Use one of: instructions, capabilities, runbooks, memory, hooks |
  </Tab>
  <Tab value="Graph">
    | Error | Cause | Fix |
    |-------|-------|-----|
    ...
    | `unconditional_cycle` | Loop with no conditional exit | Add a condition runbook to at least one edge in the cycle |
...
| References | Capabilities, instructions, runbooks, memory |
...
3. **Verify edge conditions** — confirm each conditional edge has a matching runbook.
