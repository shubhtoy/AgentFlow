...
<Mermaid chart={`graph TD
    ...
    C -->|Yes: instructions, capabilities, runbooks, memory, hooks| D[Register as resource category]
...
1. **parseRoot** performs a single directory listing of `.agentflow/`. It separates reserved directories (instructions, capabilities, runbooks, memory, hooks) from everything else. Non-reserved directories are workflows.
