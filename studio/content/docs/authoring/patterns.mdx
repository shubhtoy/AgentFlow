...
The most common pattern — a router node that blocks until a human approves. Used 8 times across the 6 library workflows.

### The files

<Tabs items={['Router SKILL.md', 'Condition runbook', 'Directory']}>
  <Tab value="Router SKILL.md">
    ```yaml
  ...
  <Tab value="Condition runbook">
    ...
    **True when:** The user accepted the design with no blocking feedback.
    **False when:** The user rejected, requested changes, or hasn't responded.
    ```
  </Tab>
  <Tab value="Directory">
    <Files>
...
- Router type is always `router` — gates don't produce artifacts
- No capabilities or instructions — routers are lightweight
- Two edges minimum — approved (forward) and rejected (backward with feedback)
- The condition runbook has `type: condition` with a `check` field
...
<Tabs items={['Gate SKILL.md', 'Condition: more tasks', 'Condition: all done']}>
  <Tab value="Gate SKILL.md">
    ...
    - If current task failed → {{-> nodes/implement-task | runbooks/task-failed}}
    - If more tasks remain → {{-> nodes/implement-task | runbooks/more-tasks-remain}}
    - If all done → {{-> nodes/verify-feature | runbooks/all-tasks-done}}
    ```
  </Tab>
  <Tab value="Condition: more tasks">
    ...
    ---
    type: condition
    name: more-tasks-remain
    check: One or more tasks in the list are not marked as done
    ---
  ...
  <Tab value="Condition: all done">
    ...
    type: condition
    name: all-tasks-done
    check: Every task completed, all tests pass, no diagnostics errors
    ---
...
A classification step feeds a router that dispatches to specialist nodes. All specialists converge at a single output node.
...
- {{runbooks/is-urgent}} → fast-track to {{nodes/respond}}
```
...
| **interactive-assistant** | Router + Conversation Loop + Memory | 8 |
...
flowchart TD
    ...
    E --> N[Router node with condition runbooks]
...
| Human must approve | Gate | Router node with condition runbooks |
| Repeat until done | Loop | Three-way router (fail/continue/done) |
