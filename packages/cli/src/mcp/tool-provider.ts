...
export const BuiltinToolRegistry: Record<string, (args: Record<string, unknown>, ctx: { workingDir: string }) => unknown> = {
  readCode(args, ctx) {
    const filePath = path.resolve(ctx.workingDir, args.path as string)
    if (!fs.existsSync(filePath)) return { error: `File not found: ${args.path}` }
    if (fs.statSync(filePath).isDirectory()) {
      return { type: 'directory', path: args.path, entries: fs.readdirSync(filePath, { withFileTypes: true }).map(e => ({ name: e.name, isDir: e.isDirectory() })) }
    }
    const content = fs.readFileSync(filePath, 'utf-8')
    if (args.symbol) {
      const matches = content.split('\n').map((line, i) => ({ line: i + 1, text: line })).filter(l => l.text.includes(args.symbol as string))
      return { path: args.path, symbol: args.symbol, matches: matches.slice(0, 30) }
    }
    const lines = content.split('\n')
    const truncated = lines.length > 500
    return { path: args.path, content: truncated ? lines.slice(0, 500).join('\n') + '\n...(truncated)' : content, lines: lines.length, truncated }
  },
  fsWrite(args, ctx) {
    const filePath = path.resolve(ctx.workingDir, args.path as string)
  ...
  getDiagnostics(args, ctx) {
    ...
    return { results: paths.map(p => {
      ...
      if (p.endsWith('.js') || p.endsWith('.mjs') || p.endsWith('.cjs')) {
        try { execSync(`node --check "${fp}"`, { stdio: 'pipe', timeout: 10000 }); return { path: p, ok: true, diagnostics: [] } }
        catch (err: unknown) { return { path: p, ok: false, diagnostics: [(err as { stderr?: Buffer }).stderr?.toString() || (err as Error).message] } }
      }
      return { path: p, ok: true, diagnostics: [], note: 'No checker available for this file type' }
    })}
  },
  webSearch(args) { return { note: 'Web search not available in orchestrator runtime', query: args.query } },
}
...
export const ScriptToolExecutor = {
  execute(command: string, args: Record<string, unknown>, ctx: { workingDir: string }) {
    ...
    try {
      ...
      return { success: false, command: cmd, exitCode: (err as { status?: number }).status, output: ((err as { stdout?: string }).stdout || '').slice(0, 5000), error: ((err as { stderr?: string }).stderr || '').slice(0, 5000) }
    }
  },
}
...
export class McpToolManager {
    ...
    for (const [name, serverEntry] of Object.entries(mcpConfig.servers || {})) {
      try {
        ...
        if (serverEntry.required) {
          const installCmd = serverEntry.command ? `${serverEntry.command} ${((serverEntry.args || []) as string[]).join(' ')}` : (serverEntry.url as string) || 'unknown'
          throw new Error(`Required MCP server "${name}" failed to start: ${(err as Error).message}\nInstall: ${installCmd}`)
        }
  ...
  private async _connectServer(name: string, serverEntry: Record<string, unknown>) {
    ...
    if (serverEntry.command) {
      transport = deps.createStdioTransport({ command: serverEntry.command as string, args: (serverEntry.args || []) as string[], env: { ...process.env, ...resolvedEnv } as Record<string, string> })
  ...
  getTools(serverName: string) { return this.servers.get(serverName)?.tools || [] }
  ...
  async execute(server: string, tool: string, args: Record<string, unknown>) {
    ...
    try { return await entry.client.callTool({ name: tool, arguments: args }) }
    catch (err: unknown) { return { error: `MCP tool "${tool}" on server "${server}" failed: ${(err as Error).message}`, isError: true } }
  }

  async shutdown() {
    for (const [, entry] of this.servers) {
      try { if (entry.client?.close) await entry.client.close() } catch { /* ignore */ }
    }
    this.servers.clear()
  }
}
...
export function buildToolEntry(key: string, toolDef: Record<string, unknown>, mcpManager: McpToolManager): ToolEntry {
  ...
  if (fm.parameters) {
    for (const [pName, pDef] of Object.entries(fm.parameters as Record<string, Record<string, unknown>>)) {
      properties[pName] = { type: pDef.type === 'array' ? 'array' : 'string', description: (pDef.description as string) || pName }
  ...
  if (toolType === 'builtin') {
    ...
    execute = builtinFn ? (args, ctx) => builtinFn(args, ctx!) : () => ({ error: `No executor for builtin: ${mapping}` })
...
export class ToolProvider {
  ...
  getToolsForNode(_node: ParsedNode, _graph: ParsedGraph): ToolEntry[] { return [] }
  async shutdown() {}
}
...
export class NodeToolProvider extends ToolProvider {
  ...
  async shutdown() { await this.mcpManager.shutdown() }
}
